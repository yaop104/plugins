...
<script language="javascript">
	...
	$(function() {
		...
		colArr = [
			...
			{ field:'tstStatementRemark', align:'center',  width:'180', sortable:'true' , title:'备注'  }
		];
		...
		grid = $('#t1').datagrid({
			...
			sortName : 'tstStatementUnid',//当数据表格初始化时以哪一列来排序
			sortOrder : 'asc',//定义排序顺序，可以是'asc'或者'desc'（正序或者倒序）
			remoteSort : false,//定义是否通过远程服务器对数据排序
			itColumns : false,
			singleSelect : false,//设置为true将只允许选择一行
			idField : 'tstStatementUnid',//表明该列是一个唯一列。
	...
	function addOption(){
		$('#d1').dialog('open');
		$('#f1').form('clear');
		$('#f1').form.url='${ctx }/TstStatement/save.do';
	}
	...
	function deleteOption(){
		...
		if(row){
			...
			$.messager.confirm('确认？', '确认删除所有选中记录吗', function(r){
				if (r){
					$.post('${ctx }/TstStatement/deleteTstStatements.do',{'ids': s},function(data){
						if(data.success){
		...
		<div id="d1" class="easyui-dialog" buttons="#btn1" title="充值"  data-options="novalidate:true,iconCls:'icon-save',closed:true,modal:true,minimizable:false" style="width:500px;height:400px;overflow: hidden;">
			<div style="padding:10px 60px 20px 60px">
				<form id="f1"  class="easyui-form" method="post">
					<input type="hidden" id="TstStatementUnid" name="TstStatementUnid"/>
					<table>
						<tr>
							<td align="right">申请充值名称：</td>
							<td><input class="easyui-validatebox" name="TstStatementMoneyid" required="true" style="width: 152px" id="TstStatementMoneyid"/></td>
						</tr>
						<tr>
							<td align="right">申请充值金额（元）：</td>
							<td><input class="easyui-validatebox" name="TstStatementAppname" required="true" style="width: 152px" id="TstStatementAppname"/></td>
						</tr>
						<tr>
							<td align="right">充值凭证：：</td>
							<td>
								<input class="easyui-validatebox" name="TstStatementUrl" required="true" style="width: 152px" id="TstStatementUrl"/>
							</td>
						</tr>
					</table>
				</form>
