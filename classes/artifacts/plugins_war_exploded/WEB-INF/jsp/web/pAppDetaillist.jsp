...
<input name="" type="button" onclick="preInsert()" value="新增记录"/>
</div>
		<div>
			<c:choose>
					...
					<table id="myTable" class="yanshi_newul"  cellpadding="0" cellspacing="0" cla="even_odd">
						<thead>
							<tr align="center">
								...
								<th nowrap width="8%" class="noSort" sortNum=2>名称</th>
								<th nowrap width="12%" class="noSort" sortNum=3>创建时间</th>
								<th nowrap width="12%" class="noSort" sortNum=4>审核状态</th>
								<th nowrap width="12%" class="noSort" sortNum=5>大小</th>
						...
						<tbody align="center">
							...
							<c:forEach var="data" items="${pageList.list}" varStatus="count">
									<td class="sortCla">${data.pAppdetailId}</td>
									<td class="sortCla">${data.pAppdetailName}</td>
									<td class="sortCla">${data.pAppdetailDate}</td>
									<c:choose>
										<c:when test="${data.pAppdetailAuditstate=='1'}">
											<td class="sortCla">通过</td>
										</c:when>
										<c:otherwise>
											<td class="sortCla">未通过</td>
										</c:otherwise>
									</c:choose>
									<td class="sortCla">${data.pAppdetailSize}</td>
									<td class="sortCla">
									<span  class="icon_chakan"><a href="javascript:showView(${data.pAppdetailId});"  title="查看详情"></a></span>
									<span  class="icon_Modify" ><a href="javascript:preUpdate(${data.pAppdetailId});" title="更新"></a></span>
									<span  class="icon_del" ><a href="javascript:del(${data.pAppdetailId});" title="删除"></a></span>
								    </td>
								</tr>
							</c:forEach>
						</tbody>
